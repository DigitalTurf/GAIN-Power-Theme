...
vc_map( array(
	...
	"base"                      => 'listingpro_pricing_custom',
	...
	"params"                    => array(
		array(
			...
			"value" => array(
				esc_html__( '', 'js_composer' ) => 'show_hide',
			),
			'save_always' => true,
...
function listingpro_shortcode_pricing_custom($atts, $content = null) {
	...
	global $listingpro_options;
		...
		$output .= '<div class="col-md-12 col-md-offset-1 padding-bottom-40 lp-margin-top-case '.$pricing_views.'">';
		...
		if($plan_status!='claim'){
			if($lp_plans_cats=='yes'){
				ob_start();
				include_once( get_stylesheet_directory() . '/templates/pricing/by_category.php' );
				$output .= ob_get_contents();
				ob_end_clean();
				ob_flush();
					ob_start();
						include_once( get_stylesheet_directory() . '/templates/pricing/'.$pricing_views.'.php');
						$output .= ob_get_contents();
					ob_end_clean();
					ob_flush();
				...
				ob_start();
				include_once( get_stylesheet_directory() . '/templates/pricing/loop/claim_plans.php');
				$output .= ob_get_contents();
				ob_end_clean();
				ob_flush();
		}
...
add_shortcode('listingpro_pricing_custom', 'listingpro_shortcode_pricing_custom');
