...
*/
	...
	if (!function_exists('Listingpro_listing_submit_init')) {
		function Listingpro_listing_submit_init(){
			wp_register_script('listingpro-submit-listing',  get_stylesheet_directory_uri() . '/assets/js/submit-listing.js', '', '' ,true );
	...
	/* ================================================================================= */
	add_action('wp_ajax_listingpro_custom_submit_listing_ajax', 'listingpro_custom_submit_listing_ajax');
	add_action('wp_ajax_nopriv_listingpro_custom_submit_listing_ajax', 'listingpro_custom_submit_listing_ajax');
	if(!function_exists('listingpro_custom_submit_listing_ajax')){
		function listingpro_custom_submit_listing_ajax(){
			...
			if($processSubmission==true){
				if ( isset( $_POST['post_nonce_field'] ) && wp_verify_nonce( $_POST['post_nonce_field'], 'post_nonce' ) ) {
					...
					/* Check if donate button is allowed */
					...
					if( !empty($errors) && count($errors)>0 ){
						...
						if ( is_user_logged_in() ) {
							...
							$listDonate = '';
							if(isset($_POST['donate']) && $donate_show){
								$listDonate = sanitize_text_field($_POST['donate']);
							}
							...
							$listMedium = sanitize_text_field($_POST['medium']);
							...
							listing_set_metabox('donate', $listDonate, $postID);
							...
							listing_set_metabox('medium', $listMedium, $postID);
							...
							//Delete originally uploaded attachments
							if(isset($_POST["business-logo-remove"])){
								if ($_POST["business-logo-remove"]) {
									$logo_ID = listing_get_metabox_by_ID('business_logo', $lp_post);
									if(is_numeric($logo_ID) && wp_attachment_is_image($logo_ID) && $post_data['post_author'] == $current_user->ID){
										listing_set_metabox('business_logo', false, $postID);
										wp_delete_attachment($logo_ID);
									}
								}
							}
							if(isset($_POST["lp-featuredimage-remove"])){
								if ($_POST["lp-featuredimage-remove"]) {
									$thumb_ID = get_post_thumbnail_id($lp_post);
									if(is_numeric($thumb_ID) && wp_attachment_is_image($thumb_ID) && $post_data['post_author'] == $current_user->ID){
										delete_post_thumbnail($lp_post);
              ...
              if ( isset($_FILES["business_logo"]) )  {
								...
								//Custom handling of business logo
								$uploadedfile = $_FILES['business_logo'];


								$fileType = wp_check_filetype($uploadedfile['name'])['type'];
								if(strpos($fileType, 'image') === false){
									die(json_encode(array('response'=>'failure', 'status'=>$response, 'dff'=>$processSubmission, 'msg'=>'Invalid file type used for your Logo.')));
									exit;
								}

								$attach_id = media_handle_upload( 'business_logo', $postID );

								if ( is_wp_error( $attachment_id ) ) {
									die(json_encode(array('response'=>'failure', 'status'=>$response, 'dff'=>$processSubmission, 'msg'=>'Unable to upload your Logo.')));
									exit;
								} else {
									listing_set_metabox('business_logo', wp_get_attachment_url($attach_id), $postID);
								}
              }
							...
							if ( isset($_FILES["lp-featuredimage"]) )  {
								...
								//Custom handling of featured image
								$uploadedfile = $_FILES['lp-featuredimage'];

								$fileType = wp_check_filetype($uploadedfile['name'])['type'];
								if(strpos($fileType, 'image') === false){
									die(json_encode(array('response'=>'failure', 'status'=>$response, 'dff'=>$processSubmission, 'msg'=>'Invalid file type for Featured Image')));
									exit;
								}

								$attach_id = media_handle_upload( 'lp-featuredimage', $postID );

								if ( is_wp_error( $attachment_id ) ) {
									die(json_encode(array('response'=>'failure', 'status'=>$response, 'dff'=>$processSubmission, 'msg'=>'Unable to upload Featured Image')));
									exit;
								} else {
									...
									if ( isset($_FILES["business_logo"]) )  {
										/*if($_FILES['business_logo']['size'] != 0) {
											...
											foreach ($files['name'] as $key => $value) {
												...
												$fileType = wp_check_filetype($uploadedfile['name'])['type'];
												if(strpos($fileType, 'image') === false){
													die(json_encode(array('response'=>'failure', 'status'=>$response, 'dff'=>$processSubmission, 'msg'=>'Invalid file type for Business Logo')));
													exit;
												}
												...
												if ( is_wp_error( $attachment_id ) ) {
													die(json_encode(array('response'=>'failure', 'status'=>$response, 'dff'=>$processSubmission, 'msg'=>'Unable to upload Business Logo')));
													exit;
												} else {
													listing_set_metabox('business_logo', wp_get_attachment_url($attach_id), $postID);
												}
				...
				if ( isset( $_POST['edit_nonce_field'] ) && wp_verify_nonce( $_POST['edit_nonce_field'], 'edit_nonce' ) ) {
					...
					if( !empty($errors) && count($errors)>0 ){
						...
						die(json_encode(array('response'=>'fail', 'status'=>$total_new_images, 'msg'=>$msg)));
					}
					else{
						...
						$listDonate = '';
						/* Check if donate button is allowed */
						$donate_show = get_post_meta( $listPlanid, 'lp_donatebutton', true );
						if($donate_show == "true"){
							$donate_show = true;
						}else{
							$donate_show = false;
						...
						if(isset($_POST["business-logo-remove"])){
							if ($_POST["business-logo-remove"]) {
								$logo_URL = listing_get_metabox_by_ID('business_logo', $lp_post);
								$logo_ID = gp_get_image_id($logo_URL);
								if(is_numeric($logo_ID) && wp_attachment_is_image($logo_ID) && $post_data['post_author'] == $current_user->ID){
									listing_set_metabox('business_logo', false, $postID);
									wp_delete_attachment($logo_ID);
								}
