...
<?php
    //Gets a random image from our custom gallery page
    $banner_image = get_post_gallery(get_banner_images_id(), false)['src'][0];
...
elseif ( $topBannerView=="banner_view_search_bottom"){
   ...
   if($topBannerView == 'banner_view2'){
    ...
    $class_list = get_body_class();
    if (in_array('has-header-img',$class_list)) {
      if( $imgClass == 'lp-header-bg' ) {
          ...
          //Checks if a banner image has already been set
          $custom_banner = get_term_meta(get_queried_object_id(),'lp_category_banner', true );
          if ($custom_banner != '' && $custom_banner != null){
              $default_banner_styles = 'background-image: url('. $custom_banner .');';
          }
          //If no banner image has been set, use a random image
          elseif (is_front_page()) {
              $default_banner_styles  =   'background-image: url('.$banner_image.');';
          }
      }
    }
