jQuery(document).ready(function($){
    ...
    //Add nice select box
    if(jQuery("#relation-select").length){
        jQuery("#relation-select").niceSelect();
    }
    ...
    jQuery( '#rewies_form' ).on('submit', function(e){
       ...
       //Gets value of relation
       var relation = jQuery("#relation-select").parent().find("span.current").html();
        if( multiState == '1' )
        {
            var $rating	=	'';
       ...
       else if(relation == '--Select relation--' || relation == ''){
           $this.find('.review_status').text("Please fill Relation, Rating, Title, and Review");
            $this.find('.review_status').addClass('error-msg');
            $this.find(':input[type=submit]').prop('disabled', false)
       }
       else{
