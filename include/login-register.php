		...
		function ajax_login(){
			...
			if($processLogin == true){
				...
				if ( is_wp_error($user_signon) ){
					...
					echo json_encode(array('loggedin'=>true, 'message'=>'<span class="alert alert-success"><i class="fas fa-check" aria-hidden="true"></i> '.esc_html__('Login successful!','listingpro').'</span>'));
				}
			}
			...
			if($processRegister == true){
				...
				if ( !$user_id && email_exists($user_email) == false && !empty($user_email) && !empty($user_name) && is_email($user_email)) {
					...
					if(!ctype_alnum($user_name)){
						$error = true;
						$note = '<span class="alert alert-danger"><i class="fa fa-times" aria-hidden="true"></i>'.esc_html__(' Symbols such as @, . cannot be used in your username/ handle. Your username will be viewable by the public. Please choose a username that represents you!','listingpro').'</span>';
					}
					if ( $error == true ){
				...
				$note = '';
				if(empty($user_pass) && $enablepassword==false){
					$note = '<span class="alert alert-success"><i class="fas fa-check" aria-hidden="true"></i> '.esc_html__('Go to your inbox or spam/junk and get your password','listingpro').'</span>';
				}
				else{
					$note = '<span class="alert alert-success"><i class="fas fa-check" aria-hidden="true"></i> '.esc_html__('Registration successfull, Please signin now','listingpro').'</span>';
				}
