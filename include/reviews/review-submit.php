<?php
	...
	function Listingpro_review_submit_init(){
		...
		wp_register_script('review-submit-ajax', get_stylesheet_directory_uri() . '/assets/js/review-submit.js', array('jquery') );
...
if(!function_exists('ajax_review_submit')){
	function ajax_review_submit(){
		...
		if($processReview==true){
			...
			$relation = stripslashes(esc_html(sanitize_text_field($_POST['relation'])));
			$user_email = '';
			$postID = '';
			...
			//Error handling for relation
			$valid_relation = false;
			$post_title = esc_html(get_the_title($listing_id));

			$possible_relations = [
				'You managed '.$post_title.' directly',
				'You reported directly to '.$post_title,
				'You were senior to '.$post_title,
				'You worked with '.$post_title.' in the same organization/campaign',
				'You worked with '.$post_title.' but in different organizations/campaigns',
				'You worked with '.$post_title.' but at different organizations/campaigns',
				$post_title.' was a client of yours',
				'You were a client of '.$post_title,
				'You mentored '.$post_title
			];

			foreach($possible_relations as $x){
				if($relation == $x){
					$valid_relation = true;
					break;
				}
			...
			if($valid_relation != true){
				$res = json_encode(array('reviewID'=>$postID, 'status'=>esc_html__('Invalid relation input', 'listingpro'), 'error'=>'input'));
				die($res);
			}
			...
			listing_set_metabox('relation', $relation, $postID);
