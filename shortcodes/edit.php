...
if(!function_exists('listingpro_shortcode_edit_custom')){
	function listingpro_shortcode_edit_custom($atts, $content = null) {
    ...
    extract(shortcode_atts(array(
		...
		$lp_imagesize_notice = '';
		if(lp_theme_option('lp_listing_images_count_switch')=='yes'){
		...
		if(isset($_GET['lp_post']) && !empty($_GET['lp_post'])){
			...
			$donate = listing_get_metabox_by_ID('donate', $lp_post);
			...
			$medium = listing_get_metabox_by_ID('medium', $lp_post);
			...
			$lp_featured_img_url = get_the_post_thumbnail_url($lp_post,'full');
			...
			$donate_show = get_post_meta( $plan_id, 'lp_donatebutton', true );
		...
		$quicktip_title = get_template_directory_uri() .'/assets/images/quick-tip/title.png';
		$quicktip_adress = get_template_directory_uri() .'/assets/images/quick-tip/contact.png';
		$quicktip_cat = get_template_directory_uri() .'/assets/images/quick-tip/cat-sub.png';
		$quicktip_price = get_template_directory_uri() .'/assets/images/quick-tip/price.png';
		$quicktip_faq = get_template_directory_uri() .'/assets/images/quick-tip/faq.png';
		$quicktip_video = get_template_directory_uri() .'/assets/images/quick-tip/video.png';
		$quicktip_gallery = get_template_directory_uri() .'/assets/images/quick-tip/gallery.png';
		$quicktip_desc = get_template_directory_uri() .'/assets/images/quick-tip/desc.png';
		$quicktip_biz = get_template_directory_uri() .'/assets/images/quick-tip/biz.png';
				...
				<div class="post-submit">
					<div class="author-section border-bottom lp-form-row clearfix lp-border-bottom padding-bottom-40">
						<div class="lp-form-row-left text-left pull-left not-logged-in-msg">
							<img src="'.listingpro_author_image() .'" alt="userimg" height="50" width="50" />
			...
			if($donate_show == "true"){
				$output .= '
									<div class="form-group ' . $style2_content_class . ' col-xs-12">
										<label for="inputDonate">Donate Button URL</label>
										<input data-quick-tip="<h2>'. esc_html__( 'Donate Button URL', 'listingpro-plugin' ) .'</h2><p>'. esc_html__( 'Where the donate button will link to', 'listingpro-plugin' ) .'</p><img src='.$quicktip_website.'>" type="text" class="form-control" name="donate" id="inputDonate" placeholder="' . esc_html__('http://', 'listingpro-plugin') . '">
									</div>';
			}
			}
      /*
			...
			if( $price_show == "true" && ($currencySwitch == 1 || $digitPriceSwitch == 1 || $priceSwitch == 1))
		   {
			...
			if ($social_show == "true" && $social_show_switch == true)
			{
				...
				if ($googleSwitch == 1) {$output .=  '<input value="'. $gPlus .'" type="hidden" class="form-control" name="google_plus" id="inputGooglePlus">';}
				...
				$output .= '<input type="hidden" value="'. $medium .'" class="form-control" name="medium" id="inputMedium">';
				...
				//Medium URL
				$output .= '<div class="social-row social-row-Medium"><label>'.esc_html__('Medium', 'listingpro-plugin').'</label><span>'. $medium .'</span><a class="remove-social-type" data-social="Medium"><i class="fa fa-times"></i></a></div>';
				...
				$output .=  '<option>' . esc_html__('Medium', 'listingpro-plugin') . '</option>';
				$output .=  '       </select>';
				$output .=  '    </div>';
			...
			{
				$output .= '<div class="white-section border-bottom ' . $style_wrap . ' media-fields">';
						...
						$output .='
											<div class="form-group clearfix margin-bottom-0 lp-img-gall-upload-section col-md-12 lplistgallery" data-quick-tip="<h2>'.esc_html__('Gallery ', 'listingpro-plugin').'</h2><img src='.$quicktip_gallery.'>" data-savedgallerysize="'.$GalimageCount.'" data-savedgallweight ="'.$galleryImagessize.'">
												<div class="col-sm-12 padding-left-0 padding-right-0">
													<label for="postVideo">'.esc_html__('Header Image(s)', 'listingpro-plugin').'</label><br>
                          <div class="small-description">We recommend you upload your logo or an image that represents you.<br>Recommended image specs: JPG or PNG, Size:  960 x 480, Max: 2MB.</div><br>
		...
		else
		{
			...
			if($tagline_show == "true"){
				$output .='
										<div class="form-group">
											<input data-quick-tip="<p>'.esc_html__('Put your listing title here and tell the name of your business to the world. ', 'listingpro-plugin').'</p>'.$quicktip_image.'" type="text" name="tagline_text" value="'.$tagline_text.'" class="form-control" id="lptagline" placeholder="' .esc_html__('Tagline Example: We power progress. ', 'listingpro-plugin').'">
										</div>';
			}
			$output .='
								</div>';
			...
			$output .='
									<div class="form-group clearfix margin-bottom-0 lp-new-cat-wrape">
										<label for="inputCategory">'.$listing_cat_text.' <small>*</small></label><br>
										<div class="small-description">Donate button is not available for Firms and Freelancers or their subcategories.</div>';
			if($singleCatMode==true){
			...
			//For donate button
			$listcats = get_the_terms( $lp_post, 'listing-category' );
			$disableStr = '';
		...
		if($donate_show == "true"){
			$output .= '
								<div class="form-group ' . $style2_content_class . ' col-xs-12 donate-form-group">
									<label for="inputDonate">Donate Button URL</label>
									<input '.$disableStr.'value="'.$donate.'" data-quick-tip="<h2>'. esc_html__( 'Donate Button URL', 'listingpro-plugin' ) .'</h2><p>'. esc_html__( 'Where the donate button will link to', 'listingpro-plugin' ) .'</p> type="text" class="form-control" name="donate" id="inputDonate" placeholder="' . esc_html__('http://', 'listingpro-plugin') . '">
								</div></div>';
		}

			$output .=				'<div class="row"><div class="form-group ' . $style2_content_class . ' col-xs-12 lp-social-area">';
			if ($social_show == "true" && $social_show_switch == true)
			{
				if($twSwitch == 1) {
					$output .='
				...
				$output .= '
				<div class="form-group col-md-6 col-xs-12">
					<label for="inputMedium">' . esc_html__('Medium', 'listingpro-plugin') . '</label>
					<input type="text" value="'.$medium.'" class="form-control" name="medium" id="inputMedium" placeholder="' . esc_html__('Your Medium URL', 'listingpro-plugin') . '">
				</div>';
			}
			$output .='
								</div>';
			...
			if ($video_show == "true" || $gallery_show == "true" || lp_theme_option('lp_featured_file_switch') || $b_logo == 1 ){
				$output .='
						<div class="white-section border-bottom '.$style_wrap.' media-fields">
							<div class="row">
								<div class="form-group '. $style2_content_class .' col-xs-12">';
				if($vdoSwitch == 1) {
					if($video_show=="true"){
           ...
           $output .= '<div class="form-group clearfix margin-bottom-0 margin-top-10 lp-listing-featuredimage">
                         <label class="margin-top-10">' . esc_html__('Directory Image', 'listingpro-plugin') . '</label><br>
                         <div class="small-description">We recommend you upload your logo or an image that represents you. <br>Recommended image specs: JPG or PNG, Size: 640 x 480, Max: 2MB.</div>
                         <div class="dropzone" id="featured-image-upload"></div>
                         <img id="placeholder-featured" class="hidden" src="'.$lp_featured_img_url.'">
                         <input type="hidden" id="featured-id" name="featured-id" value="">
                         <div id="featured-crop-wrap" style="visibility: hidden; position: absolute; height: 400px;">
                           <div id="featured-crop"></div>
                         </div>

                         <div style = "display: none; cursor: pointer;" id="featured-crop-button">Crop</div>
                         <div style = "display: none; cursor: pointer;" id="featured-remove-image">Remove</div>
                         <div class="clearfix"></div>
                         <label style="display: none;" class="margin-top-10">' . esc_html__('Banner Image', 'listingpro-plugin') . '</label>
          ...
          $output .= '<div class="form-group clearfix margin-bottom-0 margin-top-10 lp-listing-featuredimage">
										<label class="margin-top-10">' . esc_html__('Logo', 'listingpro-plugin') . '</label><br>
                    <div class="small-description">We recommend you upload your logo or an image that represents you. <br>Recommended image specs: JPG or PNG, Size: 640 x 480, Max: 2MB.</div>
										<div class="dropzone" id="business-image-upload"></div>
                    <img id="placeholder-business" class="hidden" src="'.$lp_business_logo_url.'">
										<input type="hidden" id="business-id" name="business-id" value="">
										<div id="business-crop-wrap" style="visibility: hidden; position: absolute; height: 400px;">
											<div id="business-crop"></div>
										</div>
										<div style = "display: none; cursor: pointer;" id="business-crop-button">Crop</div>
										<div style = "display: none; cursor: pointer;" id="business-remove-image">Remove</div>
										<div class="clearfix"></div>
										<div style="display: none;"class="custom-file">
											<input style="display:none;" type="file" name="business_logo[]" id="business_logo" class="inputfile inputfile-4" />
		...
		if( $page_style != 'style2' )
		{
			...
			$output .='
								...
								<div class="form-group col-md-12 margin-bottom-0">
									...
									<div class="form-group clearfix margin-bottom-0 preview-section pos-relative col-md-6 pull-right">';
...
add_shortcode('listingpro_edit_custom', 'listingpro_shortcode_edit_custom');
