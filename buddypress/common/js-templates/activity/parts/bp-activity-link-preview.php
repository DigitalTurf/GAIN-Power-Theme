<script type="text/html" id="tmpl-activity-link-preview">
	...
	<div class="activity-link-preview-container">
		...
		<div class="activity-link-preview-excerpt">
			<p class="activity-link-url">{{{data.link_url.replace('http://','').replace('https://','').replace('www.','').split(/[/?#]/)[0]}}}</p>
			<p class="activity-link-preview-title">{{{data.link_title}}}</p>
			<p class="activity-link-preview-description">{{{data.link_description}}}</p>
		</div>
		<# } #>
