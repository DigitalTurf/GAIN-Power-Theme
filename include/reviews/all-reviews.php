				...
				$args = array(
 				...
 				if ( $query->have_posts() ) {
					...
					while ( $query->have_posts() ) {
                        ...
                        //Gets the name to be shows in the relation
                        $author_display_name = get_the_author();
                        if(strpos($author_display_name, ' ') !== false){
                            $author_display_name = explode(' ', $author_display_name)[0];
                        }

                        //Gets the relation
                        $relation = listing_get_metabox_by_ID('relation' ,get_the_ID());

                        //Replace "You" and "yours" with the author's name
                        $relation = str_replace('yours', $author_display_name . "'s", $relation);
                        $relation = str_replace('You', $author_display_name, $relation);
						...
						<figure>
                            ...
                            <figcaption>
                                <h4><a href="<?php echo get_profile_url_by_id($author_id); ?>"><?php the_author(); ?></a></h4>
                                <p><i class="far fa-star"></i> <?php echo $user_reviews_count; ?> <?php esc_html_e('Reviews','listingpro'); ?></p>
						...
						<section class="details">
                            ...
                            <!-- Where the relation will be displayed -->
                            <?php if(strlen($relation) > 0){ ?>
                            <div class="relation"><i class="fas fa-project-diagram"></i><?php echo $relation; ?></div>
                            <?php } ?>
										...
										<ul>
											<li>
												<a class="instresting reviewRes" href="#" data-reacted ="<?php echo esc_html__('You already reacted', 'listingpro'); ?>" data-restype='<?php  echo $interVal; ?>' data-id='<?php the_ID(); ?>' data-score='<?php echo esc_attr($interests); ?>'>
													<i class="far fa-thumbs-up"></i><?php echo esc_html__('Interesting', 'listingpro'); ?><span class="interests-score"><?php if(!empty($interests)) echo $interests; ?></span>
											...
											<!--<li>
												<a class="lol reviewRes" href="#" data-reacted ="<?php echo esc_html__('You already reacted', 'listingpro'); ?>" data-restype='<?php echo $lolVal; ?>' data-id='<?php the_ID(); ?>' data-score='<?php echo esc_attr($Lols); ?>'>
													<i class="far fa-smile"></i><?php echo esc_html__('Lol', 'listingpro'); ?><span class="interests-score"><?php if(!empty($Lols)) echo $Lols; ?></span>
													<span class="lp_state"></span>
												</a>
											...
											<li>
												<a class="love reviewRes" href="#" data-reacted ="<?php echo esc_html__('You already reacted', 'listingpro'); ?>" data-restype='<?php echo $loveVal; ?>' data-id='<?php the_ID(); ?>' data-id='<?php the_ID(); ?>' data-score='<?php echo esc_attr($loves); ?>'>
													<i class="far fa-heart"></i><?php echo esc_html__('Love', 'listingpro'); ?><span class="interests-score"><?php if(!empty($loves)) echo $loves; ?></span>
													<span class="lp_state"></span>
												</a>
